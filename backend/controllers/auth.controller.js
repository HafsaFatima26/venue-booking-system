...
// PUT /api/auth/owner/profile
// Body: { full_name, email, phone, business_name, address, description }
// ══════════════════════════════════════════════════════════════════════════════
exports.updateOwnerProfile = async (req, res) => {
    try {
        const { full_name, email, phone, business_name, address, description } = req.body;
        await db.query(
            'UPDATE owners SET full_name=?, email=?, phone=?, business_name=?, address=?, description=? WHERE id=?',
            [full_name, email, phone, business_name, address, description, req.user.id]
        );

        const [rows] = await db.query('SELECT * FROM owners WHERE id = ?', [req.user.id]);
        if (rows.length === 0) {
            return res.status(404).json({ success: false, message: 'Owner not found.' });
        }

        return res.json({ success: true, message: 'Profile updated', user: { ...safeOwner(rows[0]), role: 'owner' } });
    } catch (err) {
        console.error('updateOwnerProfile error:', err);
